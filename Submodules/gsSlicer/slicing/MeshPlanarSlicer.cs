...
namespace gs
{
	...
	public class MeshPlanarSlicer
	{
        ...
        public PlanarSliceStack Compute(double minz ,double maxz)
        {
            if (Meshes.Count == 0)
                return new PlanarSliceStack();
        ...
        public PlanarSlice ComputeZ(double z)
        {
            ...
            for (int mi = 0; mi < Meshes.Count; ++mi)
            {
                ...
                {
                    if (useOpenMode == PrintMeshOptions.OpenPathsModes.Embedded)
                        slice.AddEmbeddedPath(pline);
                    else
                        slice.AddClippedPath(pline);
                }
            }
            ...
            out Polygon2d[] loops, out PolyLine2d[] curves )
        {
            ...
            //lxh mark
            // [RMS] debug visualization try to fill and export svg
            //if (Math.Abs(z - 0.42) < 0.1)
            //{
            //    DGraph2 graph2 = new DGraph2();
            //    Dictionary<int, int> mapV = new Dictionary<int, int>();
